...
The modeling approach uses a Markov cohort simulation, the standard method in health economics for chronic progressive diseases (technical details in Sections II.B-C). We calibrated the model to reproduce observed natural history from Japanese and international cohorts (Ando et al. 2024; Zaniew et al. 2018): starting with eGFR of 83 ml/min/1.73m² at age 1 and declining at age-varying rates (1.0 ml/min/year ages 1-9, accelerating to 3.5 ml/min/year ages 10-19, then 2.0 ml/min/year ages 20+), patients reach end-stage kidney disease at median age 32 and die at median age 35 (range 20-40 years). Model calibration validated via 1,000-patient Monte Carlo simulation matching published natural history targets. Critically, we decompose total kidney decline into normal aging (0.3 ml/min/year constant) and pathological disease-related decline (age-varying). Gene therapy can only affect the pathological component; it cannot slow biological aging. This mathematical framework (D_treated = D_age + (1-θ)×D_path, detailed in Section II.D equations 6-7) ensures our scenarios remain biologically plausible.
