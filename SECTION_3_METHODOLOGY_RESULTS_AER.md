...
**Treatment Effect Scenarios.** Absent clinical trial data, we model three treatment effect scenarios based on predicted enzyme restoration levels, informed by the carrier biology observation that female carriers expressing approximately 50 percent of normal OCRL enzyme typically remain free of progressive renal dysfunction (Charnas et al. 2000; Bökenkamp & Ludwig 2016; Röschinger et al. 2000). We posit a linear dose-response relationship between enzyme restoration (E) and decline reduction (θ):

(4)    θ = E + 0.35

where θ represents the proportional reduction in eGFR decline rate and E is the achieved enzyme restoration as a proportion of normal levels. This formulation implies a 35 percent baseline treatment benefit plus linear scaling with enzyme activity, calibrated such that carrier-level enzyme restoration (E = 0.50) yields prevention of progressive kidney disease analogous to the typical carrier phenotype (θ = 0.85). While rare cases of severe phenotype occur in carriers despite 50% enzyme activity—likely due to skewed X-inactivation (Yamamoto et al. 2019)—the vast majority demonstrate no renal progression, supporting this as the primary scenario. The three modeled scenarios span the plausible efficacy range:
